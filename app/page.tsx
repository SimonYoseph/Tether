...
import { CircleUserRound, Edit3, Link2, LoaderCircle, LogOut, Moon, Plus, Search, Sun, Tag, X } from "lucide-react";
...
export default function Home() {
  ...
  return <main className="app-shell" style={{ "--ui-font": fontOptions[fontChoice] } as CSSProperties}>
    <aside className="side-rail"><Link className="rail-logo" href="/" aria-label="Tether home"><Image src="/tether.jpg" alt="" width={46} height={46} priority /></Link><button className="rail-add" onClick={() => setAddOpen(true)}><Plus size={22} /><span>Add note</span></button><div className="rail-bottom"><button className="icon-button" onClick={toggleTheme} title={dark ? "Use light mode" : "Use dark mode"} aria-label={dark ? "Use light mode" : "Use dark mode"}>{dark ? <Sun size={17} /> : <Moon size={17} />}</button><span className="rail-version">v1.0</span></div></aside>
    <div className="dashboard"><header className="site-header"><Link className="brand" href="/"><span className="brand-wordmark">TETHER</span></Link><div className="header-actions">{user ? <button className="header-link" onClick={() => void supabase.auth.signOut()}><LogOut size={14} /> Sign out</button> : <a className="header-link" href="#account">Sign in</a>}<button className="header-profile" aria-label="Account"><CircleUserRound size={18} /></button></div></header>
      <section className="search-header"><div className="subheader-line"><span>Your Thought Space</span><b>|</b><strong>Keep the Good Stuff.</strong></div><div className="search-tools"><div className="global-search"><Search size={18} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search by text or date..." /><kbd>/</kbd></div><button className="add-note-button" onClick={() => setAddOpen(!addOpen)} aria-label="Add a note" title="Add a note"><Plus size={20} /></button></div><div className="tag-tools"><button><Tag size={15} /> Add Tag</button><button><Edit3 size={15} /> Edit Tags</button><label className="font-picker">Font<select value={fontChoice} onChange={(event) => setFontChoice(event.target.value as keyof typeof fontOptions)} aria-label="Choose interface font"><option value="rounded">Rounded</option><option value="humanist">Humanist</option><option value="editorial">Editorial</option><option value="mono">Mono</option><option value="geometric">Geometric</option></select></label>{allTags.map((tag) => <span key={tag}>#{tag}</span>)}</div></section>
      <div className="connection-strip"><span className="connection-icon"><Link2 size={16} /></span><div><strong>Keep your thoughts close</strong><small>Capture ideas, notes, and links in one quiet place.</small></div><span className="connected-state">Connected</span></div>
      ...
      <footer><span>Ideas have somewhere to land.</span><span>Free to use. Yours to keep.</span></footer></div>
  </main>;
}
